...
export default function CivicRequestComposer() {
  ...
  return (
    <section
      className="fixed bottom-4 left-4 right-4 z-50 font-['Inter',sans-serif] min-[861px]:left-[272px] min-[861px]:right-6"
      aria-label="Civic request composer"
    >
      <form
        onSubmit={handleSubmit}
        className="mx-auto flex h-[58px] w-full max-w-[820px] items-center gap-1 rounded-full border border-[#3B4D5D] bg-[#263746]/[0.98] px-2 shadow-[0_10px_30px_rgba(0,0,0,0.20)] backdrop-blur-xl transition-all duration-200 focus-within:border-[#526779]"
      >
        <input
          ref={fileInput}
          className="sr-only"
          id="home-request-file"
          type="file"
        ...
        <div className="flex shrink-0 items-center gap-0.5">
          <button
            ...
            className="grid h-9 w-9 place-items-center rounded-full text-white transition-all duration-150 hover:bg-[#35495A] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2D7FF9]/25"
          ...
          <button
            ...
            className={`grid h-9 w-9 place-items-center rounded-full transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2D7FF9]/25 ${
              isListening
                ? "bg-[#D8E9FA] text-[#2D7FF9]"
                : "text-white hover:bg-[#35495A]"
            }`}
          ...
          <span className="ml-0.5 grid h-9 w-9 place-items-center rounded-full border border-[#506273] bg-[#334655]" aria-hidden="true">
            <CivicAIIcon />
          </span>
        </div>

        {fileName && (
          <span className="hidden max-w-[130px] truncate rounded-full border border-[#526779] px-2 py-1 text-[10px] font-medium text-[#DCE8F5] sm:block">
            {fileName}
          </span>
        )}
        ...
        <textarea
          ...
          rows={1}
          className="min-w-0 flex-1 resize-none overflow-hidden border-0 bg-transparent px-2 py-1 text-[13px] font-medium leading-5 text-white outline-none placeholder:text-[#8394A5]"
        />

        <button
          type="submit"
          disabled={!request.trim()}
          className={`grid h-9 w-9 shrink-0 place-items-center rounded-full transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2D7FF9]/25 ${
            request.trim()
