...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#FAFAFC] font-['Inter',sans-serif] text-[#0D1B2A]">
      ...
      >
        <div>
          ...
          <div className="px-6 pt-4 pb-2">
            <span className="inline-block rounded bg-[#2D7FF9]/20 px-2.5 py-1 text-[10px] font-extrabold tracking-[0.16em] text-[#8DBBFF] uppercase">
              Gov Admin Portal
            </span>
          </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3.5 rounded-lg px-4 py-3 text-xs font-bold tracking-[0.06em] uppercase transition-all ${
                    isActive
        ...
        <div className="p-4 border-t border-white/10">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-lg px-4 py-3 text-xs font-extrabold tracking-[0.12em] text-[#FF5252] hover:bg-red-500/10 uppercase transition-all"
          >
            <span>➔ Logout</span>
          </button>
        </div>
      ...
      <main className="flex flex-1 flex-col overflow-y-auto">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-[#D6E6F7] bg-white px-6 py-4 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="hidden items-center gap-2 rounded-lg bg-[#2D7FF9] px-4 py-2 text-xs font-black tracking-[0.12em] text-white uppercase shadow-md hover:bg-[#1E4FA3] sm:flex"
            >
              + Issue Public Notice
            </button>
        ...
        <div className="p-6 space-y-6">
          ...
          {/* VIEW 4: GEOGRAPHY TAB */}
          {activeNav === "geography" && <Geography />}
          ...
          {/* VIEW 8: OTHER TABS */}
          {activeNav !== "overview" && activeNav !== "queries" && activeNav !== "clusters" && activeNav !== "geography" && activeNav !== "data" && activeNav !== "projects" && activeNav !== "ai_insights" && (
      ...
      {showBroadcastModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0D1B2A]/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-[#D6E6F7] bg-white p-6 shadow-2xl">
            <h3 className="text-xl font-black text-[#0D1B2A]">Issue Public Notice</h3>
            <form onSubmit={handleBroadcastSubmit} className="my-4 space-y-3">
              <textarea
                required
                rows="3"
                value={broadcastText}
                ...
                className="w-full rounded-xl border border-[#D6E6F7] p-3 text-sm outline-none focus:border-[#2D7FF9]"
              />
              {broadcastSuccess && (
                <p className="text-xs font-bold text-emerald-600">✓ Notice broadcasted to citizen portal!</p>
              )}
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowBroadcastModal(false)} className="px-3 py-1.5 text-xs font-bold border rounded-lg">Cancel</button>
                <button type="submit" className="px-3 py-1.5 text-xs font-extrabold text-white bg-[#2D7FF9] rounded-lg">Broadcast</button>
              </div>
            </form>
          </div>
        </div>
